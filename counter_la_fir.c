...
void main()
{
	...
	reg_mprj_datal = 0x00A50000; // Start mark ('hA5)
...
*/
	...
	for (int i = 0; i < N; i++){
		...
		//(*(reg_fir_coeff+(volatile uint32_t *)(4*i)))=*(tap+i) ;	
		//(*(volatile uint32_t*)(reg_fir_coeff + 4*i)) = *(tap + i);
	}
	...
	for (int m = 0; m < 3; m++){
		...
		if(m==3){
			...
			reg_mprj_datal = 0x00AB0000;}
	}

	// Y data
	int Y = reg_fir_y;

	if (Y == 0x2A2D) 		// Y[7:0]-> mprj[31:24], End mark= 'h5A 
	//if (Y == 0x044A)     
		reg_mprj_datal = (Y << 24) | 0x005A0000;

	// print("\n");
	// print("Monitor: Test 1 Passed\n\n");	// Makes simulation very long!
	//reg_mprj_datal = 0x4A5A0000;
}
